...
export default function IngressesPage() {
  ...
  const { data = [], loading, refresh } = useK8sResource('ingresses', namespace);
  const permissions = usePermissions('ingresses');
  ...
  const handleDelete = async (record: any) => {
    ...
    const res = await request(`/api/k8s/${clusterId}/namespaces/${ns}/ingresses/${name}`, { method: 'DELETE' });
    if (res.ok) { message.success(`Ingress ${name} 已删除`); refresh(); }
        ...
        return (
          <Space>
            ...
            {permissions.canDelete && !system && (
              <DeleteConfirm name={record.metadata?.name} kindLabel="Ingress" onConfirm={() => handleDelete(record)} />
            )}
          </Space>
        );
      },
