...
export default function DaemonSetsPage() {
  ...
  const { data = [], loading, refresh } = useK8sResource('daemonsets', namespace);
  const permissions = usePermissions('daemonsets');
  ...
  const handleDelete = async (record: any) => {
    ...
    const res = await request(`/api/k8s/${clusterId}/namespaces/${ns}/daemonsets/${name}`, { method: 'DELETE' });
    if (res.ok) { message.success(`DaemonSet ${name} 已删除`); refresh(); }
  ...
  const columns = [
    ...
    { title: '命名空间', dataIndex: ['metadata', 'namespace'], key: 'namespace' },
    {
      ...
      key: 'ready',
      render: (_: any, r: any) =>
        `${r.status?.numberReady || 0}/${r.status?.desiredNumberScheduled || 0}`,
    },
    {
      title: '状态',
      key: 'status',
      render: (_: any, r: any) => {
        const ready = (r.status?.numberReady || 0) === (r.status?.desiredNumberScheduled || 0);
        ...
        return (
          <Space>
            ...
            {permissions.canDelete && !system && (
              <DeleteConfirm name={record.metadata?.name} kindLabel="DaemonSet" onConfirm={() => handleDelete(record)} />
            )}
          </Space>
        );
      },
