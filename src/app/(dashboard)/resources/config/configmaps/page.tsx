...
export default function ConfigMapsPage() {
  ...
  const handleDelete = async (record: any) => {
    ...
    const res = await request(`/api/k8s/${clusterId}/namespaces/${ns}/configmaps/${name}`, { method: 'DELETE' });
    if (res.ok) { message.success(`ConfigMap ${name} 已删除`); refresh(); }
        ...
        return (
          <Space>
            ...
            {permissions.canDelete && !system && (
              <DeleteConfirm name={record.metadata?.name} kindLabel="ConfigMap" onConfirm={() => handleDelete(record)} />
            )}
          </Space>
        );
      },
